...
namespace NavigationRoutes {
	...
	public static class NavigationRouteConfigurationExtensions {
		public static void MapNavigationRoute(this RouteCollection routes,
			...
			var newRoute = new NamedRoute(name,
			                              url,
			                              new MvcRouteHandler()) {
		...
		public static NavigationRouteBuilder MapNavigationRoute(this RouteCollection routes,
		                                                        string name,
		                                                        string displayName,
		                                                        string url,
		                                                        object defaults,
			...
			routes.Add(name,
			           newRoute);
			return new NavigationRouteBuilder(routes,
			                                  newRoute);
		}
		...
		public static NavigationRouteBuilder MapNavigationRoute<T>(this RouteCollection routes,
		                                                           string displayName,
		                                                           Expression<Func<T, ActionResult>> action,
		                                                           string areaName = "") where T : IController {
			var newRoute = new NamedRoute("",
			                              "",
			                              new MvcRouteHandler());
			newRoute.ToDefaultAction(action,
			                         areaName);
			newRoute.DisplayName = displayName;
			routes.Add(newRoute.Name,
			           newRoute);
			return new NavigationRouteBuilder(routes,
			                                  newRoute);
		}

		public static NavigationRouteBuilder AddChildRoute<T>(this NavigationRouteBuilder builder,
		                                                      string DisplayText,
		                                                      Expression<Func<T, ActionResult>> action,
		                                                      string areaName = "") where T : IController {
			var childRoute = new NamedRoute("",
			                                "",
			                                new MvcRouteHandler());
			...
			builder._routes.Add(childRoute.Name,
			                    childRoute);
			return builder;
		}

		public static NamedRoute ToDefaultAction<T>(this NamedRoute route,
		                                            Expression<Func<T, ActionResult>> action,
		                                            string areaName) where T : IController {
			...
			var attributes = body.Method.GetCustomAttributes(typeof (ActionNameAttribute),
			                                                 false);
			...
			foreach (var pair in route.Defaults.Where(x => x.Value == null).
			                           ToList())
				route.Defaults.Remove(pair.Key);

			route.Defaults.Add("controller",
			                   controllerName);
			route.Defaults.Add("action",
			                   actionName);

			route.Url = CreateUrl(actionName,
			                      controllerName,
			                      areaName);
			...
			route.DataTokens.Add("Namespaces",
	...
	public class NavigationRouteBuilder {
		public NavigationRouteBuilder(RouteCollection routes,
		                              NamedRoute parent) {
			_routes = routes;
			_parent = parent;
		}
